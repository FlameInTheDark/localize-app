...
export function LanguageSelect({ value, onChange, label = "Target language", className }: { value: string; onChange: (value: string) => void; label?: string; className?: string }) {
  ...
  const isKnownCode = languages.some((language) => language.code === normalizedQuery);
  ...
  return <Popover.Root open={open} onOpenChange={setOpen}>
    <Popover.Trigger asChild>
      <button type="button" role="combobox" aria-label={label} aria-expanded={open} className={cn("language-trigger language-trigger-ghost", className)}>
        <span className="flex min-w-0 items-center gap-2"><Globe2 className="size-4 shrink-0 text-muted-foreground" /><span className="truncate">{selected?.name ?? "Select language"}</span>{selected && <span className="language-code">{selected.code}</span>}</span>
