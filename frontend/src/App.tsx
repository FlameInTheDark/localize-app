...
import { ArrowLeft, Download, FileText, Image as ImageIcon, ScanText, Settings, Type } from "lucide-react";
...
export function App() {
  ...
  return <div className="app-frame relative flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <main className="flex min-h-0 w-full flex-1 flex-col p-2 sm:p-3"><div className="flex h-full min-h-0 w-full flex-1 flex-col"><div className="min-h-0 flex-1">{settingsOpen ? <Suspense fallback={<div className="surface h-full animate-pulse" />}><SettingsView theme={theme} onThemeChange={setTheme} version={applicationVersion} /></Suspense> : <>{tab === "text" && <TextWorkspace seed={ocrText} clearSeed={() => setOCRText("")} />}{tab === "document" && <DocumentTranslator dropPath={dropFor("document")} />}{tab === "image" && <ImageTranslator dropPath={dropFor("image")} />}{tab === "ocr" && <OCRExtractor dropPath={dropFor("ocr")} onSend={(text) => { setOCRText(text); setTab("text"); }} />}</>}</div></div></main>
