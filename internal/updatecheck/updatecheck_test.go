...
func TestCheckerCheck(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			checker := NewChecker(fakeSource{release: Release{Version: test.latest, URL: "https://example.invalid/release"}}, test.current)
...
func TestGitHubSourceLatest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		if request.Header.Get("User-Agent") != "Localize-update-checker" {
			t.Errorf("User-Agent = %q", request.Header.Get("User-Agent"))
		}
		writer.Header().Set("Content-Type", "application/json")
		_, _ = writer.Write([]byte(`{"tag_name":"v1.2.3","html_url":"https://github.com/FlameInTheDark/localize-app/releases/tag/v1.2.3"}`))
	}))
	defer server.Close()
