// Package updatecheck compares Localize's release version with GitHub's latest release.
package updatecheck
...
const (
	defaultLatestReleaseURL = "https://api.github.com/repos/FlameInTheDark/localize-app/releases/latest"
	defaultRequestTimeout   = 12 * time.Second
	maxResponseBytes        = 1 << 20
)

// Release identifies a published Localize version and its GitHub page.
type Release struct {
	Version string
	URL     string
}
...
// Checker compares a running application version to releases supplied by Source.
type Checker struct {
	source  Source
...
// NewChecker creates a release checker for a running application version.
// Development builds without a semantic version intentionally skip checks.
...
// GitHubSource retrieves the latest published Localize GitHub release.
...
func (s *GitHubSource) Latest(ctx context.Context) (release Release, err error) {
	...
	req.Header.Set("User-Agent", "Localize-update-checker")
